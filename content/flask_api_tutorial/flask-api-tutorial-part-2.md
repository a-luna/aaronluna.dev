...
class User(db.Model):
    ...
    def registered_on_str(self):
        return self.registered_on.replace(tzinfo=timezone.utc).strftime(DT_STR_AWARE)
...
[In a previous post](/2019/02/06/error-handling-python-result-class/), I demonstrated and explained the merits of incorporating principles from functional programming, with the `Result` class as a useful example. We will use this class frequently in this API project, so please read the linked post. When you finish that, create a new file in the `util` folder named `result.py` and add the content below:
